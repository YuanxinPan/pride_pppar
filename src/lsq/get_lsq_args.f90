...
if(ierr.ne.0) then
  write(*,'(2a)') '***ERROR(get_lsq_args): open file ',sesfil(1:len_trim(sesfil))
  call exit(1)
endif
!
!! start & stop time
...
key=findkey(lfn,msg,' ')
if(key(1:5).eq.'EMPTY') goto 100
LCF%ztdmod=key(1:len_trim(key))
!
!! Horizontal Troposphere Gradients
msg='HTG model'
key=findkey(lfn,msg,' ')
if(key(1:5).eq.'EMPTY') goto 100
LCF%htgmod=key(1:len_trim(key))
!
!! Rinex directory
msg='Rinex directory'
key=findkey(lfn,msg,' ')
if(key(1:5).eq.'EMPTY') goto 100
rnxpath=key(1:len_trim(key))
i=len_trim(rnxpath)
...
return
100 continue
write(*,'(3a)') '***ERROR(get_lsq_args): find option ',msg(1:len_trim(msg)),key(1:len_trim(key))
call exit(1)
200 continue
write(*,'(3a)') '***ERROR(get_lsq_args): read option ',msg(1:len_trim(msg)),key(1:len_trim(key))
call exit(1)
end
