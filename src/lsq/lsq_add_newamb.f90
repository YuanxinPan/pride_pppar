...
nbias=0
do isat=1,LCF%nprn
  if(LCF%prn(isat).eq.0.or.OB%obs(isat,1).eq.0.d0) cycle
...
!! set living time for ambiguity parameters
    if(OB%lifamb(isat,2).ne.0.d0) then
      ...
      write(oscr,'(a,i3,1x,a,i5,4i3,f11.7)') '***ERROR(lsq_add_newamb): living ',OB%prn(isat),name//' at',iy,imon,id,ih,im,sec
      call exit(1)
    endif
!
!! set initial value
    call read_iniambi(LCF%flnamb,name,OB%prn(isat),OB%lifamb(isat,1),OB%lifamb(isat,2),ambini,rwini,ierr)
    if(ierr.eq.0) then
      PM(NM%ipm+nbias)%xini=ambini
      PM(NM%ipm+nbias)%zw  =rwini
    endif
  endif
enddo
