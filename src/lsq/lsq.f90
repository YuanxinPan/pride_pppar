...
!! observation
  ...
  if(ierr.ne.0) then
    write(*,'(a)') '***ERROR(lsq): open file ',SITE%obsfil(1:len_trim(SITE%obsfil))
    call exit(1)
  endif
  call rdrnxoh(SITE%iunit,HD,ierr)
  if(ierr.ne.0) then
    write(*,'(2a)') '***ERROR(lsq): read header ',SITE%obsfil(1:len_trim(SITE%obsfil))
    call exit(1)
  endif
...
!! save a priori horizontal troposphere gradients
    if(LCF%htgmod(1:3).ne.'NON'.and.LCF%htgmod(1:3).ne.'FIX') then
      ipar=pointer_string(OB%npar,OB%pname,'HTGC'//LCF%htgmod(1:len_trim(LCF%htgmod)))
      PM(OB%ltog(ipar,1))%xini=OB%nhtg
      ipar=pointer_string(OB%npar,OB%pname,'HTGS'//LCF%htgmod(1:len_trim(LCF%htgmod)))
...
!! save a priori zenith troposphere delay for each epoch
    if(LCF%ztdmod(1:3).ne.'FIX') then
      ipar=pointer_string(OB%npar,OB%pname,'ZTD'//LCF%ztdmod(1:len_trim(LCF%ztdmod)))
      ipar=OB%ltog(ipar,1)
