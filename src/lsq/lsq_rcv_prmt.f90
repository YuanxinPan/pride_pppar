...
do while(.true.)
  ...
  if(cid.eq.'ob') then
    backspace lfnobs
    read(lfnobs) jd,mw,isit,isat,ntot,ipt(0),(ipt(k),cof(k),k=1,ntot),phase,range,wphs,wrng,iflg,elev,azim,dmap,wmap
    dummy=0.d0
    do i=1,ntot
    ...
    backspace lfnrem
    read(lfnrem) ipar,ntot,ipt(0),(ipt(k),k=1,ntot-1),(cof(k),k=1,ntot),PM(ipar)%iobs,PM(ipar)%xini,PM(ipar)%ptime(1:2)
    dummy=cof(ipt(0))
    ...
    if(PM(ipar)%iobs.gt.0) then
      ...
      if(PM(ipar)%pname(1:6).eq.'RECCLK') then
        ...
        write(tmphtg) SITE%name,PM(ipar)%ptime(1:2),(PM(ipar+i)%xini,PM(ipar+i)%xcor,i=0,1)
          ...
          write(tmpkin) PM(ipar)%pcode(1),int(mw),(mw-int(mw))*864.d2,(PM(ipar+i)%xini+PM(ipar+i)%xcor,i=0,2),PM(ipar)%iobs
    ...
    if((dummy-PM(ipar)%ptime(1))*86400.d0.gt.-MAXWND.and.(PM(ipar)%ptime(2)-dummy)*86400.d0.gt.-MAXWND.and.&
        PM(ipar)%iobs.gt.0) then
