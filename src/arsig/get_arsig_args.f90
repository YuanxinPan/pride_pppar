...
!
!! local
integer*4 nargs,i,lfn,iprn,iy,imon,id,ih,im,ierr
real*8 is,seslen
character*20 sesfil
...
if(ierr.ne.0) then
  write(oscr,'(2a)') '***ERROR(get_arsig_args): open file ',sesfil(1:len_trim(sesfil))
  call exit(1)
endif
FCB%flnwlf=' '
!
...
close(lfn)
return
100 write(oscr,'(3a)') '***ERROR(get_arsig_args): find option ',msg(1:len_trim(msg)),key(1:len_trim(key))
call exit(1)
200 write(oscr,'(3a)') '***ERROR(get_arsig_args): read option ',msg(1:len_trim(msg)),key(1:len_trim(key))
call exit(1)
end
