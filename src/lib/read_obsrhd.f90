...
integer*4 jd,nprn,prn(1:*)
real*8 sod
type(rnxobr) OB
!
!! local
integer*4 i,iprn,iy,imon,id,ih,im,jdx,jde,ierr
real*8 dt,sec,sodx,sode,dintv
character*256 line
!
!! function called
integer*4 get_valid_unit,pointer_int,modified_julday
real*8 timdif
!
!! header
...
if(OB%rhdfil(1:1).ne.' '.and.OB%lfnrhd.eq.0) then
  ...
  if(ierr.ne.0) then
    write(oscr,'(2a)') '***ERROR(read_obsrhd): open file ',OB%rhdfil(1:len_trim(OB%rhdfil))
    call exit(1)
  endif
  line=' '
  OB%amb_epo=0
...
do while(.true.)
  ...
  if(dt.gt.MAXWND) then
    ...
    do while(.true.)
      ...
      read(line,'(1x,i2)') iprn
      i=pointer_int(nprn,prn,iprn)
      if(i.eq.0) cycle
...
return
100 write(oscr,'(a)') '***ERROR(read_obsrhd): read file ',OB%rhdfil(1:len_trim(OB%rhdfil))
call exit(1)
end
