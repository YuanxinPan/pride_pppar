...
integer*4 jd,jd1,nprn,prn(1:*)
real*8 sod,sod1,dintv,xsp3(6,MAXSAT)
character*(*) fln
!
!! local
logical*1 lfirst
integer*4 iy,im,id,ih,imin,nwk,mjd,i,j,k,l,lfn,ierr,iflag
integer*4 nepo,jd0,nprn0,prn0(MAXSAT*2),jdx
real*8 fmjd,sec,sod0,x,y,z,t,dt,sodx
character*80 :: line=''
character*1 :: sys(MAXSAT*2),cid=''
!
!! function called
integer*4 modified_julday,pointer_int,get_valid_unit
real*8 timdif
...
if(lfirst) then
  ...
  if(ierr.ne.0) then
    write(oscr,'(2a)') '***ERROR(rdsp3h): open file ',fln(1:len_trim(fln))
    call exit(1)
  endif
  jd0=0
  sod0=0.d0
...
!
!! only GPS satellites are allowed
    do i=1,nprn0
      if((sys(i).eq.'G'.or.sys(i).eq.' ').and.pointer_int(nprn,prn,prn0(i)).eq.0) then
        nprn=nprn+1
        prn(nprn)=prn0(i)
      endif
    enddo
