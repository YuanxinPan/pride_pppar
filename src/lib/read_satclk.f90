...
if(lfirst) then
  ...
  if(ierr.ne.0) then
    write(oscr,'(2a)') '***ERROR(read_satclk) : open file ',clkfil(1:len_trim(clkfil))
    call exit(1)
  endif
  dintv=30.d0      
  line=' '
...
return
100 write(oscr,'(2a)') '***ERROR(read_satclk) : read file ',clkfil(1:len_trim(clkfil))
call exit(1)
200 write(oscr,'(2a)') '***ERROR(read_satclk) : end of file ',clkfil(1:len_trim(clkfil))
call exit(1)
end
