...
if(lfirst) then
  ...
  if(ierr.eq.0) then
    write(oscr,'(2a)') '%%%MESSAGE(read_igserp): ERP read ',erpfil(1:len_trim(erpfil))
  else
    write(oscr,'(2a)') '***ERROR(read_igserp): open file ',erpfil(1:len_trim(erpfil))
    call exit(1)
  endif
  read(lfn,'(a)') line
...
100 write(oscr,'(a)') '***ERROR(read_igserp): end of file igserp'
    call exit(1)
200 write(oscr,'(2a)') '***ERROR(read_igserp): read file igserp ',line(1:len_trim(line))
    call exit(1)
!
!! reset
Entry igserp_reset()
