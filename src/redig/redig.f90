...
!! function called
  logical*1 istrue
  integer*4 modified_julday, pointer_int, find_ambd
  real*8 timdif
!
!! instruction
  write (*, '(a)') '++++++++++++++++++++++++++++++++++++'
...
!! read original health diagnose info
  ...
  do while (.true.)
    ...
    do while (.true.)
      ...
      read (line, *) iprn
      isat = pointer_int(RCF%nprn, RCF%prn, iprn)
      if (isat .eq. 0) cycle
  ...
  do iepo = 1, MAXEPO
    ...
    do isat = 1, RCF%nprn
      if (rhd(isat, iepo) (1:3) .eq. 'AMB') then
        ...
        write (str(i), '(1x,i2,28x,i5,4i3,f11.7,1x,a)') RCF%prn(isat), iy, imon, id, ih, im, sec, 'AMB'
      else if (rhd(isat, iepo) (1:3) .eq. 'DEL') then
        i = i + 1
        write (str(i), '(1x,i2,57x,a)') RCF%prn(isat), rhd(isat, iepo) (1:len_trim(rhd(isat, iepo)))
      endif
    enddo
    if (i .eq. 0) cycle
    tx = tb + (iepo - 1)*intv
