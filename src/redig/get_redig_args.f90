...
if(ierr.ne.0) then
  write(oscr,'(2a)') '***ERROR(get_redig_args): open file ',resfil(1:len_trim(resfil))
  call exit(1)
endif
line=' '
RCF%snam=' '
...
if(ierr.ne.0) then
  write(oscr,'(2a)') '***ERROR(get_redig_args): open file ',line(1:len_trim(line))
  call exit(1)
endif
RCF%lupd=.false.

return
100 write(oscr,'(2a)') '***ERROR(get_redig_args): read arguments ',line(1:len_trim(line))
    call exit(1)
end
