...
if(ierr.ne.0) then
  write(oscr,'(2a)') '***ERROR(get_sp3orb_args): open file ',erpfil(1:len_trim(erpfil))
  call exit(1)
endif
line=' '
...
if(ierr.ne.0) then
  write(oscr,'(2a)') '***ERROR(get_sp3orb_args): open file ',sescfg(1:len_trim(sescfg))
  call exit(1)
endif
!
!! time tag
...
return
100 continue
write(oscr,'(3a)') '***ERROR(get_sp3orb_args): find option ',msg(1:len_trim(msg)),key(1:len_trim(key))
call exit(1)
200 continue
write(oscr,'(3a)') '***ERROR(get_sp3orb_args): read option ',msg(1:len_trim(msg)),key(1:len_trim(key))
call exit(1)
end
