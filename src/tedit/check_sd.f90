...
!-nobsflg : # of flags (amb.) for each satellite
  do iprn=1,nsat
    ...
    if(nobsprn(iprn).gt.ndgr+2) then
      ...
      enddo
      call check_for_jump(' LG ',lfnsd,kobs,ti(i0),lg,flglg,ndgr,niter,2.0d0,0.40d0,a0,rmslg(iprn),vlg,ilg,ierr,interval)
      if(ierr.ne.0) then
...
!!-range_ref : distance from ref. satellites to static station in each epoch
  ...
  enddo
  call check_for_jump(' LGR ',lfnsd,kobs,ti(i0),lg,flglg,ndgr,niter,2.0d0,0.40d0,a0,rmslg(iref),vlg,ilg,ierr,interval)
  if(ierr.ne.0) then
  ...
  do iprn=1,nsat
    if(nobsprn(iprn).gt.ndgr+2.and.iprn.ne.iref) then
      ...
      if(nsd.gt.ndgr+2) then
        ...
        call check_for_jump(' LC ',lfnsd,kobs,ti(i0),lc,flglc,ndgr,niter,0.2d0,0.12d0,a0,rmslc(iprn),vlc,ilc,ierr,interval)
        if(ierr.eq.0) then
