...
! local
  character*256 arg(20, 6), arg0, message, chr*1
...
!! read arguments
  ...
  do while (ipar .le. npar .and. .not. display_help)
    ...
    if (arg(ipar, 1) (1:5) .eq. '-rnxn') then
      ...
      display_help = nval(ipar) .ne. 4
      if (.not. display_help) then
        read (arg(ipar, 2), *, iostat=ioerr) x1
        read (arg(ipar, 3), *, iostat=ioerr) x2
        read (arg(ipar, 4), *, iostat=ioerr) x3
        display_help = ioerr .ne. 0
      endif
