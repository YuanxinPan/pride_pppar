...
# ------------------------------------------------------------------
# Purpose   : GPS Standard Precise Point Positioning(PPP)
...
if (! -e "${tbdir}$ctrl_file") {
    print "\n control_file($ctrl_file) does not exit!\n";
    exit;
}
...
while($k <= $end_mjd){
    ...
    # Get tables, sp3 and rinex data path
...
sub turboedit {
    ...
    my $dirrnx  = `get_ctrl $ctrl_file "Rinex directory"`;
    my $dirbrdc = `get_ctrl $ctrl_file "Rinex directory"`;
    $dirrnx     = substr($dirrnx,0,(length($dirrnx)-1));
    $dirbrdc    = substr($dirbrdc,0,(length($dirbrdc)-1));
    ...
    while($numsite < scalar(@sitename)) {
        ...
        my $y= '';
        my $z= '';
        open($flhdl, "sit.xyz") or die "Cannt open PANDA site file : sit.xyz" ;
        seek($flhdl,0,0);
        ...
        if(! -e $logflnam){
            #print "$rinexdata $sitetype[$numsite]"; #=============
...
sub sp3orb_staclk{
    ...
    if(! -e "sck_${year4}${doy}"){
        ...
        #if(! -e "cck_${year4}${doy}"){
        if(! -e "cck${week}${dow}.clk"){
            `$wget -nv -nc -t 3 --connect-timeout=10 --read-timeout=60 ftp://192.168.111.19/cck${week}${dow}.clk`;
        ...
        rename("cck${week}${dow}.clk","sck_${year4}${doy}");
    }
    if(! -e "fcb_${year4}${doy}"){
        if(! -e "$FCB"){
